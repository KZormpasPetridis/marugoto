...
def deploy_categorical_model_(
    ...
    *,
    ...
    output_path = Path(output_path)
    if (preds_csv := output_path / "patient-preds.csv").exists():  # MV: filename changed from patient-preds.csv to patient-preds-testset.csv
    ...
    *,
    ...
    for fold, (train_idxs, test_idxs) in enumerate(folds):
        ...
        if (preds_csv := fold_path / "patient-preds.csv").exists():
            ...
            fold_test_df = df.iloc[test_idxs]  # MV: added
            learn = _crossval_train(
                fold_path=fold_path,
                fold_df=df,  # MV2
                fold_train_df=fold_train_df,  # MV2
                fold_test_df=fold_test_df,  # MV: added
                fold=fold,
                info=info,
...
def _crossval_train(
    *, fold_path, fold_df, fold_train_df, fold_test_df,fold, info, target_label, target_enc, cat_labels, cont_labels
):
    """Helper function for training the folds."""
    assert fold_df.PATIENT.nunique() == len(fold_df)
    assert fold_train_df.PATIENT.nunique() == len(fold_train_df)  # MV2
    assert fold_test_df.PATIENT.nunique() == len(fold_test_df)  # MV: ADDED
    ...
    train_distr = {k: int(v) for k, v in fold_train_df[target_label].value_counts().items()}  # MV2
    valid_distr = {k: int(v) for k, v in fold_test_df[target_label].value_counts().items()}
    info["class distribution"][f"fold {fold}"] = {
        "overall": {'1': train_distr['1'] + valid_distr['1'],
                    '0': train_distr['0'] + valid_distr['0']}
    }

    # train_patients, valid_patients = train_test_split(
    #     fold_df.PATIENT, stratify=fold_df[target_label], random_state=1337
    # )
    train_patients = fold_train_df["PATIENT"]  # MV Series con nome soggetti. Attenzione agli indici
    valid_patients = fold_test_df["PATIENT"]  # MV Series con nome soggetti. Attenzione agli indici
    ...
    learn = train(
        bags=fold_df.slide_path.values,  # MV2: all the elements in the development set
